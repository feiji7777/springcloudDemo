...
//	    	ASN1ObjectIdentifier sm2DataOidPub = (ASN1ObjectIdentifier) pubSeq.getObjectAt(0);
    ...
    public static Sm2Cert readSm2File1(byte[] pem, String pwd){
...
//	    	ASN1ObjectIdentifier sm4AlgOid = (ASN1ObjectIdentifier) priSeq.getObjectAt(1);
            ...
            byte[] key =Sm4Utils.getSecretKey(pwd);

            //KDF(pwd.getBytes(), 128);
           byte[] priKeyD1 = sm4DecryptCBC(Arrays.copyOfRange(key, 16, 32),
                    priKeyAsn1.getOctets(),
                    Arrays.copyOfRange(key, 0, 16), SM4_CBC_NOPADDING);

            byte[] priKeyD = Sm4Utils.decryptEcb(key,priKeyAsn1.getOctets());

            String privateKeys = Hex.toHexString(priKeyD);
            System.out.println(privateKeys);
            System.out.println(Hex.toHexString(priKeyD1));

            sm2Cert.privateKey = getPrivatekeyFromD(new BigInteger(1, priKeyD1));
//	    	System.out.println(Hex.toHexString(priKeyD));

//	    	ASN1ObjectIdentifier sm2DataOidPub = (ASN1ObjectIdentifier) pubSeq.getObjectAt(0);
           /* ASN1OctetString pubKeyX509 = (ASN1OctetString) asn1Sequence.getObjectAt(1);
            CertificateFactory cf = CertificateFactory.getInstance("X.509", "BC");
            X509Certificate x509 = (X509Certificate) cf.generateCertificate(new ByteArrayInputStream(pubKeyX509.getOctets()));
            System.out.println(Base64.encodeBase64String(x509.getEncoded()));
            sm2Cert.publicKey = x509.getPublicKey();
...
//        System.out.println(Hex.toHexString(bs));
       ...
       String sm1 = "";
        byte[] bs = Base64.decodeBase64(sm1);
        String pwd = "cfca1234";
        String pwd1 = "4041572446";
        GmUtil.Sm2Cert sm2Cert = GmUtil.readSm2File1(bs, pwd1);
        System.out.println(sm2Cert.getPublicKey());
        System.out.println("p="+Hex.toHexString(((BCECPrivateKey)sm2Cert.getPrivateKey()).getD().toByteArray()));
        System.out.println("privateKey="+sm2Cert.getPrivateKey());
        System.out.println(sm2Cert.getCertId());

        byte[] x509bs = Base64.decodeBase64("");
